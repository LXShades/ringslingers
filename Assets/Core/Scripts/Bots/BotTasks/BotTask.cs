...
public class BT_CollectAndShoot : IBotTask
{
    ...
    public void Update(in BotTaskParams taskParams, ref CharacterInput input)
    {
        BotController controller = taskParams.controller;
        Character character = taskParams.character;
        if (!isCollecting)
        {
            Character target = SelectTarget(character, out bool targetIsVisible);

            controller.DeactivateState<BT_CollectRings>();
...
public class BT_MoveTowards : IBotTask
{
    ...
    public void Update(in BotTaskParams taskParams, ref CharacterInput input)
    {
        ...
        botController = taskParams.controller;
    }
...
public class BT_FollowPlayer : IBotTask
{
...
public class BT_CollectRings : IBotTask
{
    ...
    public void Update(in BotTaskParams taskParams, ref CharacterInput input)
    {
        ...
        if (rings.Count == 0)
        {
            ...
            rings.AddRange(Ring.FindObjectsByType<Ring>(FindObjectsSortMode.None).Select(x => new CachedRing()
            {
                ring = x,
...
public class BT_CopyPlayer : IBotTask
{
