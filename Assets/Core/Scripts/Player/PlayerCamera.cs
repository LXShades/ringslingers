        ...
        if (currentPlayer)
        {
            ...
            if (currentPlayer.movement.isOnGround)
            {
                ...
                if (landBobTimer > 0)
                {
                    ...
                    landBobTimer = Mathf.Max(landBobTimer - GameState.live.deltaTime, 0);
                }

                transform.position += Vector3.up * (Mathf.Sin(eyeBobSpeed * GameState.live.time * Mathf.Deg2Rad) * eyeBobHeight * Mathf.Min(1, currentPlayer.movement.velocity.Horizontal().magnitude / maxPlayerVelocityForEyeBob));
            }
